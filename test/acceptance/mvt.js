...
function mvtTest(usePostGIS) {
    ...
    [256, 4096, 10000, 4096 * Math.pow(2,18)]
    .forEach(size => {
...
function describe_compare_renderer() {
    ...
    LAYER_TESTS.forEach(test => {
        it(test.name, function (done) {
            ...
            const testClientMapnik = new TestClient(test.mapConfig, clientOptions);
            const testClientPg_mvt = new TestClient(test.mapConfig, clientOptions);
